...
class TestScraperCoordinator:
    ...
    def sample_events(self) -> List[Event]:
        ...
        future_date = datetime.now(PACIFIC_TZ) + timedelta(days=2)
        return [
            Event(
            ...
            Event(
                ...
                title="Test Truck 2",
                datetime_start=future_date.replace(hour=13),
    ...
    async def test_scrape_one_success_filters_and_returns(
        ...
        past_event = Event(
            ...
            venue_name="Single Test Brewery",
            title="Past Truck",
            datetime_start=past_date.replace(tzinfo=PACIFIC_TZ),
        )
    ...
    async def test_filter_and_sort_events(
        ...
        past_event = Event(
            ...
            venue_name="Test",
            title="Past Event",
            datetime_start=now - timedelta(days=1),  # Yesterday
        )
        future_event_1 = Event(
            venue_key="test",
            venue_name="Test",
            title="Future Event 1",
            datetime_start=now + timedelta(days=2, hours=12),  # Day after tomorrow
            datetime_end=now + timedelta(days=2, hours=20),
        )
        future_event_2 = Event(
            venue_key="test",
            venue_name="Test",
            title="Future Event 2",
            datetime_start=now + timedelta(days=1, hours=12),  # Tomorrow
            datetime_end=now + timedelta(days=1, hours=20),
        )
        far_future_event = Event(
            venue_key="test",
            venue_name="Test",
            title="Far Future Event",
            datetime_start=now + timedelta(days=10),  # Too far in future
        )
    ...
    def test_scraping_error_creation(self) -> None:
        ...
        venue = Venue("test", "Test Brewery", "https://example.com", source_type="html")
        error = ScrapingError(
            venue=venue,
    ...
    def test_has_errors(self, coordinator: ScraperCoordinator) -> None:
        ...
        # Add an error
        venue = Venue("test", "Test", "https://example.com", source_type="html")
        error = ScrapingError(venue, "Test", "Test message")
