...
def mock_events() -> List[Dict[str, Any]]:
    """Fixture providing mock food truck events (new wire format)."""
    return [
        {
            "venue_key": "test-brewery-1",
            "venue_name": "Test Brewery 1",
            "title": "Test Truck 1",
            "datetime_start": "2025-07-06T13:00:00-07:00",
            "datetime_end": "2025-07-06T20:00:00-07:00",
        ...
        },
        {
            ...
            "venue_name": "Test Brewery 1",
            "title": "AI Truck",
            "datetime_start": "2025-07-07T13:00:00-07:00",
            "datetime_end": None,
            "description": None,
...
class TestScrapeActivities:
        ...
        with patch(
            ...
            mock_food_truck_events = [
                Event(
                    ...
                    venue_name="Test Brewery 1",
                    title="Test Truck 1",
                    datetime_start=datetime(2025, 7, 6, 13, 0, tzinfo=PACIFIC_TZ),
                    datetime_end=datetime(2025, 7, 6, 20, 0, tzinfo=PACIFIC_TZ),
                    description="Great food truck",
                    extraction_method="html",
                ),
                Event(
                    ...
                    title="AI Truck",
                    datetime_start=datetime(2025, 7, 7, 13, 0, tzinfo=PACIFIC_TZ),
                    datetime_end=None,
                    description=None,
                    extraction_method="ai-vision",
                ),
            ...
            assert "2025-07-06" in event1["datetime_start"]
            assert "13:00:00" in event1["datetime_start"]
            assert "20:00:00" in event1["datetime_end"]
            ...
            assert "2025-07-07" in event2["datetime_start"]
            assert event2["datetime_end"] is None
    ...
    async def test_scrape_food_trucks_with_errors(
        ...
        with patch(
            ...
            assert (
                error["user_message"]
                == "Failed to fetch information for venue: Test Brewery 1"
            )
    ...
    async def test_generate_web_data(self, mock_events: List[Dict[str, Any]]) -> None:
        ...
        with patch(
            ...
            payload = {
                ...
                "errors": [
                    {
                        "user_message": "Failed to fetch information for venue: Test Brewery 1"
                    }
                ],
            }
