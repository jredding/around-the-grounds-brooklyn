...
class TestObecBrewingParser:
    ...
    @pytest.mark.asyncio
    @patch("around_the_grounds.parsers.obec_brewing.now_in_pacific")
    async def test_parse_valid_food_truck_info(
        ...
        mock_now.return_value = datetime(2025, 7, 5, 14, 30, tzinfo=ZoneInfo("America/Los_Angeles"))
        ...
        with aioresponses() as m:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                assert event.datetime_start.month == 7
                assert event.datetime_start.day == 5
                assert event.datetime_start is not None
                assert event.datetime_start.hour == 16  # 4pm
                assert event.datetime_end is not None
                assert event.datetime_end.hour == 20  # 8pm
    ...
    async def test_parse_different_time_format(
        ...
        with aioresponses() as m:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                assert event.datetime_start is not None
                assert event.datetime_start.hour == 12  # 12:30pm
                assert event.datetime_start.minute == 30
                assert event.datetime_end is not None
                assert event.datetime_end.hour == 18  # 6pm
                assert event.datetime_end.minute == 0
    ...
    async def test_parse_invalid_time_format(self, parser: ObecBrewingParser) -> None:
        ...
        with aioresponses() as m:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                assert event.datetime_start is not None  # always set to today's date
    ...
    async def test_parse_custom_pattern(self, parser: ObecBrewingParser) -> None:
        ...
        with aioresponses() as m:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                assert event.datetime_start is not None
                assert event.datetime_start.hour == 23
                assert event.datetime_end is not None
                assert event.datetime_end.hour == 15
    ...
    async def test_parse_extra_whitespace(self, parser: ObecBrewingParser) -> None:
        ...
        with aioresponses() as m:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                assert event.datetime_start is not None
                assert event.datetime_start.hour == 12
                assert event.datetime_end is not None
                assert event.datetime_end.hour == 16
