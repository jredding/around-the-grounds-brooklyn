...
class WheeliePopParser(BaseParser):
    ...
    def _parse_food_truck_article(
        ...
        ds = start_time if start_time is not None else date.replace(tzinfo=PACIFIC_TZ)
        return Event(
    ...
    def _parse_time(self, article: Tag, selector: str) -> Optional[datetime]:
        ...
        return utc_to_pacific_aware(parsed)
    ...
    def _event_key(self, event: Event) -> str:
        start_key = event.datetime_start.strftime("%Y-%m-%d %H:%M")
        return f"{event.datetime_start.strftime('%Y-%m-%d')}|{start_key}|{event.title.lower()}"
